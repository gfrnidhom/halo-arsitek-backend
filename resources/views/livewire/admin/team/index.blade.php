<div class="p-4 md:p-8">
    ...
    <div class="mb-4 flex items-center justify-between">
        <label class="flex items-center gap-2 text-sm text-[var(--admin-text-secondary)] cursor-pointer">
            <input wire:model.live="selectAll" type="checkbox" class="w-4 h-4 rounded border-[var(--admin-border)] bg-[var(--admin-bg-page)] text-[var(--admin-primary)] focus:ring-[var(--admin-primary)]">
            Select All
        </label>
    </div>
            ...
            <div class="bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-2xl overflow-hidden hover:border-[var(--admin-border)]/50 transition-all flex flex-col">
                <div class="aspect-square bg-slate-200 dark:bg-slate-700 relative overflow-hidden">
                    <div class="absolute top-3 left-3 z-10">
                        <input wire:model.live="selectedIds" value="{{ $tm->id }}" type="checkbox" class="w-4 h-4 rounded border-[var(--admin-border)] bg-[var(--admin-bg-page)] text-[var(--admin-primary)] cursor-pointer">
                    </div>
                    @if($tm->image)
    ...
    @if($showBulkDeleteModal)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-2xl p-6 max-w-sm w-full shadow-sm">
                ...
                <p class="text-sm text-[var(--admin-text-secondary)] mb-6">Are you sure you want to delete {{ count($selectedIds) }} selected team member(s)?</p>
                <div class="flex gap-3">
