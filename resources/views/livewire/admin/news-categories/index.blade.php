<div class="p-4 md:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-lg font-semibold text-[var(--admin-text-primary)]">News Categories</h2>
            <p class="text-xs text-[var(--admin-text-secondary)] mt-0.5">Manage topics and classifications for articles</p>
        </div>
            ...
            <div class="bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-2xl p-6 max-w-md w-full shadow-sm">
                ...
                <form wire:submit="save" class="space-y-4">
                    <div>
                        ...
                        <input wire:model.live.debounce.300ms="name" type="text" placeholder="Arsitektur, Tips, dll"
                    ...
                    <div>
                        ...
                        <input wire:model="slug" type="text" placeholder="arsitektur"
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-[var(--admin-border)]">
                        ...
                        <th class="text-center px-6 py-3.5 text-[11px] text-[var(--admin-text-secondary)] uppercase tracking-wider font-medium">Articles</th>
                ...
                <tbody class="divide-y divide-[var(--admin-border)]">
                    @forelse($categories as $category)
                        <tr class="hover:bg-[var(--admin-hover-bg)] transition-colors" wire:key="cat-{{ $category->id }}">
                            <td class="px-4 py-4 text-center">
                                <input wire:model.live="selectedIds" value="{{ $category->id }}" type="checkbox" class="w-4 h-4 rounded border-[var(--admin-border)] bg-[var(--admin-bg-page)] text-[var(--admin-primary)] cursor-pointer" {{ $category->news_count > 0 ? 'disabled' : '' }}>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $category->news_count > 0 ? 'bg-blue-100 dark:bg-blue-900/40 text-[var(--admin-primary-text)]' : 'bg-slate-200 dark:bg-slate-700 text-[var(--admin-text-secondary)]' }}">
                                    {{ $category->news_count }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-sm text-[var(--admin-text-secondary)]">No news categories found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($showBulkDeleteModal)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-2xl p-6 max-w-sm w-full shadow-sm">
                <h3 class="text-lg font-semibold text-[var(--admin-text-primary)] mb-1">Bulk Delete</h3>
                <p class="text-sm text-[var(--admin-text-secondary)] mb-6">Are you sure you want to delete {{ count($selectedIds) }} selected category(ies)?</p>
                <div class="flex gap-3">
