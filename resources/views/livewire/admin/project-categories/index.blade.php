                    ...
                    @forelse($categories as $category)
                        <tr class="hover:bg-[var(--admin-hover-bg)] transition-colors" wire:key="cat-{{ $category->id }}">
                            <td class="px-4 py-4 text-center">
                                <input wire:model.live="selectedIds" value="{{ $category->id }}" type="checkbox" class="w-4 h-4 rounded border-[var(--admin-border)] bg-[var(--admin-bg-page)] text-[var(--admin-primary)] cursor-pointer" {{ $category->projects_count > 0 ? 'disabled' : '' }}>
                            </td>
                            <td class="px-6 py-4">
