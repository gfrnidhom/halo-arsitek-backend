                    ...
                    @forelse($newsList as $news)
                        <tr class="hover:bg-[var(--admin-hover-bg)] transition-colors" wire:key="news-{{ $news->id }}">
                            <td class="px-4 py-4 text-center">
                                <input wire:model.live="selectedIds" value="{{ $news->id }}" type="checkbox" class="w-4 h-4 rounded border-[var(--admin-border)] bg-[var(--admin-bg-page)] text-[var(--admin-primary)] cursor-pointer">
                            </td>
                            <td class="px-6 py-4">
    ...
    @if($showBulkDeleteModal)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-2xl p-6 max-w-sm w-full shadow-sm">
                ...
                <p class="text-sm text-[var(--admin-text-secondary)] mb-6">Are you sure you want to delete {{ count($selectedIds) }} selected article(s)?</p>
                <div class="flex gap-3">
